...
!!---------------------------setPoi----------------------------!!
  ...
  implicit none
    ...
    if(nn.gt.nnMax)then
      write(tf,'(" [ERR] numNei > numNeiMax",2I10)')nn,nnMax
      stop
    endif
...
!!---------------------------mls2DDx---------------------------!!
  ...
  implicit none
    ...
    if(abs(ADet).lt.1e-10)then
      write(tf,'(" [ERR] ADet is too small",F15.6)')ADet
      write(tf,'(" [---] Location X Y",2F15.6)')x,y
      write(tf,'(" [---] Num Neigh",I10)')nn
      return
    endif    
...
!       write(tf,'(" [ERR] C0 is too small",F15.6)')c0
!       write(tf,'(" [---] Location X Y",2F15.6)')xi,yi
!       write(tf,'(" [---] Num Neigh",I10)')nn
!       stop
!     endif
